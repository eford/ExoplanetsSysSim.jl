...
KeplerTargetObs(n::Integer) = KeplerTargetObs( fill(TransitPlanetObs(),n), fill(TransitPlanetObs(),n), ObservedSystemDetectionProbsEmpty(),  falses(has_sc_bit_array_size), StarObs(0.0,0.0) )
...
function calc_target_obs_sky_ave(t::KeplerTarget, sim_param::SimParam)
  ...
  star_obs = StarObs( t.sys[1].star.radius, t.sys[1].star.mass )  # NOTE:  This sets the observed star properties to be those in the stellar catalog.  If want to incorporate uncertainty in stellar properties, that would be done elsewhere when translating depths into planet radii.  
