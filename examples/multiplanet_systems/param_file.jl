...
function set_test_param(sim_param_closure::SimParam)
    ...
    #rate_tab_init = reshape(rate_tab_init, (length(p_lim_arr_num)-1)*(length(r_lim_arr_num)-1))
  ...
  add_param_active(sim_param_closure,"sigma_hk",0.03)
  add_param_active(sim_param_closure,"sigma_incl",1.0)   # degrees; 0 = coplanar w/ generate_kepler_target_simple; ignored by generate_planetary_system_uncorrelated_incl
...
function set_test_param(sim_param_closure::SimParam)
    return sim_param_closure
    add_param_fixed(sim_param_closure,"num_targets_sim_pass_one",5000)  # For faster simulated catalogs
    p_lim_arr_num = [10., 20.]
