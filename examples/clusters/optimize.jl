...
add_param_fixed(sim_param,"num_targets_sim_pass_one",78005) #9)   # For "observed" data, use a realistic number of targets (after any cuts you want to perform)

model_name = "Clustered_P_R_broken_R_optimization" #"Clustered_P_R_broken_R_simulated_optimization"
...
tic()
opt_result = bboptimize(target_function_Kepler_weighted; SearchRange = active_params_box, NumDimensions = length(active_param_true), Method = :adaptive_de_rand_1_bin_radiuslimited, PopulationSize = PopSize, MaxFuncEvals = max_evals, TargetFitness = mean_weighted_dist, FitnessTolerance = std_weighted_dist, TraceMode = :verbose)
t_elapsed = toc()
