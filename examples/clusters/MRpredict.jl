...
function conditional_density(y::Float64, y_max::Float64, y_min::Float64, x_max::Float64, x_min::Float64, deg::Int64, w_hat::Vector{Float64} ; y_sd::Union{Void,Float64}=nothing, qtl::Vector{Float64}=[0.16, 0.84]) ###is 'y_sd=nothing' a good choice?
    deg_vec = 1:deg
    ...
    denominator = sum(w_hat .* kron(ones(deg), y_beta_indv)::Vector{Float64})

    #Mean:
    #mean_beta_indv = deg_vec/(deg+1)*(x_max-x_min)+x_min
    #mean_beta = kron(deg_vec/(deg+1)*(x_max-x_min)+x_min, y_beta_indv)
    #mean_numerator = sum(w_hat .* kron(deg_vec/(deg+1)*(x_max-x_min)+x_min, y_beta_indv))
    ...
    #var_beta = kron(deg_vec .* (deg-deg_vec+1) / ((deg+1)^2*(deg+2))*(x_max-x_min)^2, y_beta_indv)
    #var_numerator = sum(w_hat .* kron(deg_vec .* (deg-deg_vec+1) / ((deg+1)^2*(deg+2))*(x_max-x_min)^2, y_beta_indv))
    ...
    function pbeta_conditional_density(x::Float64)::Float64
        function mix_density(j::Float64)::Float64
            x_indv_cdf::Array{Float64,1} = map(degree -> cdf.(Beta(degree, deg-degree+1), (j-x_min)/(x_max-x_min)), deg_vec)
            quantile_numerator = sum(w_hat .* kron(x_indv_cdf, y_beta_indv)::Vector{Float64})
            return quantile_numerator/denominator
            #return sum(w_hat .* kron(map(degree -> cdf.(Beta(degree, deg-degree+1), (j-x_min)/(x_max-x_min)), deg_vec), y_beta_indv)) / denominator
        end
    ...
    function conditional_quantile(q::Float64)::Float64
        ...
        result = optimize(g2, x_min, x_max, GoldenSection(), rel_tol=1e-3, abs_tol=1e-3)
...
Radii = ones(1000)*ExoplanetsSysSim.earth_radius
tic()
