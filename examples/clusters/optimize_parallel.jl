...
for (i,param_key) in enumerate(active_param_keys)
    if ~in(i,transformed_indices)
        active_param_draw = active_params_transformed_box[i][1] + (active_params_transformed_box[i][2] - active_params_transformed_box[i][1])*rand(1)
...
println(f, "# Optimization active parameters search bounds: ", active_params_transformed_box)
...
tic()
opt_result = bboptimize(active_params_transformed -> target_function_transformed_params(active_params_transformed, transformed_indices, transformed_triangle[1], transformed_triangle[2], transformed_triangle[3], use_KS_or_AD, Kep_or_Sim ; AD_mod=AD_mod, weights=weights, all_dist=false, save_dist=true); SearchRange = active_params_transformed_box, NumDimensions = length(active_param_true), Method = :dxnes, PopulationSize = PopSize, MaxFuncEvals = max_evals, TargetFitness = target_fitness, FitnessTolerance = target_fitness_std, TraceMode = :verbose, Workers = workers())
t_elapsed = toc()
