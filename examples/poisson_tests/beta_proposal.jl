...
function make_proposal_dist_multidim_beta_single_rp(theta::AbstractArray{Float64,2}, weights::AbstractArray{Float64,1},  tau_factor::Float64; verbose::Bool = false)
    global sim_param_closure
    const p_dim = length(get_any(EvalSysSimModel.sim_param_closure, "p_lim_arr", Array{Float64,1}))-1
    const r_dim = length(get_any(EvalSysSimModel.sim_param_closure, "r_lim_arr", Array{Float64,1}))-1
    const max_col_rate = 3.0
...
function make_proposal_dist_multidim_beta_single_rp(pop::abc_population_type, tau_factor::Float64; verbose::Bool = false)
    make_proposal_dist_multidim_beta_single_rp(pop.theta, pop.weights, tau_factor, verbose=verbose)
end
